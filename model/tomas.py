def build_network_tomas(config, input_var=None):
    ...
    network = ConvLayer(network, num_filters=250, filter_size=1, nonlinearity=rectify)
    print("C1", network.output_shape)
    network = ConvLayer(network, num_filters=100, filter_size=1, nonlinearity=rectify)
    print("C2", network.output_shape)
    network = PoolLayer(network, pool_size=5, ignore_border=True)
    ...
    network = PoolLayer(network, pool_size=3, ignore_border=True)
